  ...
  methods: {
    onSubmit (e) {
      ...
      if (this.loginForm.errors > 0) {
        // 表单验证没通过时，不继续往下执行，即不会通过 axios 调用后端 API
        return false
      }

      const path = 'http://localhost:5000/api/tokens'
      // axios 实现 Basic Auth 需要在 config 中设置 auth 这个属性即可 ？？？
      axios.post(path, {}, {
        auth: {
          ...
          // handle success
          console.log('返回token：', response.data.token)
          window.localStorage.setItem('wablog-token', response.data.token)
